...
export const GET = async (req: Request) => {
    try {
        ...
        const baseHref = new URL(
            `/stake?validator=${validator.toBase58()}`,
            requestUrl.origin,
        ).toString();

        const payload: ActionGetResponse = {
            title: "Stake your SOL",
            icon: new URL("/validator-image.png", requestUrl.origin).toString(),
            description: `Stake it with validator.com. Don't lose to inflation. Don't stake it... and you might be saying, "You want fries with that?"`,
            ...
            links: {
                actions: [
                    ...
                    {
                        label: "Stake 2 SOL", // button text
                        href: `${baseHref}&amount=${"2"}`,
                    },
                    {
                        label: "Stake 3 SOL", // button text
